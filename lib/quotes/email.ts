...
// Builds the short, branded email that delivers a quote: a warm, inviting line
// (NO price — the client sees their pricing INSIDE the quote via the
// configurator), a clear "View your quote" button to the tracked link, and the
// sales contact. HTML themed to the business's brand colour, with a plain-text
...
export function buildQuoteEmail(args: {
  ...
  const { businessName, brand, clientName, quoteNumber, link } = args;
  ...
  // NO price anywhere (no headline total, no tier prices) — the client opens the
  // quote to explore the options and see their pricing there (calm configurator,
  // not a number in the inbox).
  const text = [
    `Hi ${who},`,
    ``,
    `Thanks for the opportunity. Your personalised quote${quoteNumber ? ` (${quoteNumber})` : ""} from ${biz} is ready — open it to explore your options and see your pricing.`,
    ``,
    `View your quote: ${link}`,
    ``,
    `Inside, you can tailor your selections, download a PDF and accept online.`,
    ``,
    contact,
    ``,
    biz,
    ...
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background:#ffffff;border-radius:10px;overflow:hidden;font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:${ink}">
        ...
        <tr><td style="padding:8px 32px 0;font-size:15px;line-height:1.6;color:#3a342c">
          ...
          <p style="margin:0 0 14px">Thanks for the opportunity. Your personalised quote${quoteNumber ? ` <strong>${esc(quoteNumber)}</strong>` : ""} from ${esc(biz)} is ready — open it to explore your options and see your pricing.</p>
          <p style="margin:0 0 22px">Inside, you can tailor your selections, download a PDF and accept online.</p>
        </td></tr>
