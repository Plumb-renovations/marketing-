...
import {
  Zap, X, CalendarDays, RotateCcw, Plus, TrendingUp, MapPin,
} from "lucide-react";
...
function LeadCard({ lead, onClick }: { lead: Lead; onClick: () => void }) {
  ...
  return (
    <button onClick={onClick} className="group w-full rounded-xl border border-slate-800 bg-slate-900 p-3 text-left transition hover:border-cyan-500/40 hover:bg-slate-800/60">
...
function StageColumn({ stage, leads, onCard }: { stage: (typeof STAGES)[number]; leads: Lead[]; onCard: (id: string) => void }) {
  ...
  return (
    <div className="flex w-72 shrink-0 flex-col">
      ...
      <div className="flex flex-1 flex-col gap-2 rounded-2xl border border-slate-800/70 bg-slate-950/40 p-2">
        ...
        {leads.map((l) => <LeadCard key={l.id} lead={l} onClick={() => onCard(l.id)} />)}
      </div>
    </div>
  );
}
...
export default function LeadsScreen() {
  const { leads, setSelId, setAddOpen, aiDismissed, setAiDismissed, resetBoard } = useData();
  ...
  return (
    <div>
      ...
      <div className="flex gap-3 overflow-x-auto pb-4">
        {STAGES.map((s) => <StageColumn key={s.id} stage={s} leads={shown.filter((l) => l.stage === s.id)} onCard={setSelId} />)}
      </div>
    </div>
  );
}
